...
public class DnevnikVezbanjaResource {
    ...
    public String pregledPodataka() {
        ...
        if (session != null && session.getAttribute("user") != null) {
            ...
            List<Object[]> podaci = DB.query("SELECT t.treDatum, t.treTrajanje, t.trePuls, t.treIzdrzljivost, v.vezNaziv, st.strTrajanje, st.strbrSerija, st.strbrPonavljanja,  p.posEmail "
    ...
    public String upisPodataka(String content) {
        ...
        if (session != null && session.getAttribute("user") != null) {
            ...
            String user = session.getAttribute("user") + "";
            List<Posetilac> posetioci = DB.query("SELECT p FROM Posetilac p WHERE p.posEmail=?1 ", user);
    ...
    public void upisPodatakaOVezbi(String content) {
        ...
        if (session != null && session.getAttribute("user") != null) {
            ...
            int idTrening = 0;
            int anaerobna = 0;
            List<Vezba> Vezbe = DB.query("SELECT v FROM Vezba v WHERE v.vezNaziv=?1 ", naziv);
            for (Vezba v : Vezbe) {
                idVezbe = v.getVezId();
            ...
            String user = session.getAttribute("user") + "";
            List<Trening> trening = DB.query("SELECT t FROM Trening t JOIN Posetilac p ON t.posId.posId = p.posId "
